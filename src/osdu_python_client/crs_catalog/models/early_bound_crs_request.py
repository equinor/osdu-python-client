...
T = TypeVar("T", bound="EarlyBoundCRSRequest")


@_attrs_define
class EarlyBoundCRSRequest:
    """Request to get one EarlyBoundCRS given its 'persistableReference' (serialized essence) or 'essence' structure. Only
    one, persistableReference or essence must be provided. If both are provided, essence takes precedence

        Attributes:
            essence (EarlyBoundCRSEssenceImpl | Unset): Early-bound coordinate reference system, i.e. a CRS with a
                transformation binding to WGS 84
            persistable_reference (str | Unset): The persistable reference string, either the essence of the EarlyBoundCRS
                serialized into a JSON string or an encoded string (version 1)
    """

    essence: EarlyBoundCRSEssenceImpl | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        essence: EarlyBoundCRSEssenceImpl | Unset
        if isinstance(_essence, Unset):
            essence = UNSET
        else:
            essence = EarlyBoundCRSEssenceImpl.from_dict(_essence)

        persistable_reference = d.pop("persistableReference", UNSET)

        early_bound_crs_request = cls(
            essence=essence,
            persistable_reference=persistable_reference,
        )

        early_bound_crs_request.additional_properties = d
        return early_bound_crs_request
