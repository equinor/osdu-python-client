...
) -> AppError | CTResults | None:
    if response.status_code == 200:
        response_200 = CTResults.from_dict(response.json())
...
def sync_detailed(
    *,
    ...
    """Search Cartographic Transformations
     ...
     Search cartographic transform records (any sub-type) by keywords and/or geographic area.
    Geographical constraints are passed (optionally) as parameters via the query URL, search keywords
    are passed via the request body. Query syntax follows Lucene style, default operator for multiple
    expressions is AND. The following keywords are supported: ''name:'',''description:'', ''type:'',
    ''authority:'', ''code:'', ''deprecationState:'', ''remarks:'', ''lastModified:'', ''source:'',
    ''wellKnownText:'', ''fromCRS.authority:'', ''fromCRS.code:'', ''toCRS.authority:'', toCRS.code:'',
    ''policy:''. followed by a value. Example: ''fromCRS.code:4267''.
