...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = CRS.from_dict(response.json())
...
def sync_detailed(
    *,
    ...
    Args:
        ...
        data_partition_id (str):
        body (CRSRequest): Request to get one CRS (any sub-type) given its 'persistableReference'
            (serialized essence) or 'essence' structure. Only one, persistableReference or essence
            must be provided. If both are provided, essence takes precedence. If both are provided,
            essence takes precedence
