...
T = TypeVar("T", bound="LateBoundCRSRequest")


@_attrs_define
class LateBoundCRSRequest:
    """Request to get one LateBoundCRS given its 'persistableReference' (serialized essence) or 'essence' structure. Only
    one, persistableReference or essence must be provided. If both are provided, essence takes precedence.

        Attributes:
            essence (LateBoundCRSEssenceImpl | Unset): Late-bound coordinate reference system, i.e. a CRS without a
                transformation binding to WGS 84
            persistable_reference (str | Unset): The persistable reference string, either the essence of the LateBoundCRS
                serialized into a JSON string or an encoded string (version 1).
    """

    essence: LateBoundCRSEssenceImpl | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        essence: LateBoundCRSEssenceImpl | Unset
        if isinstance(_essence, Unset):
            essence = UNSET
        else:
            essence = LateBoundCRSEssenceImpl.from_dict(_essence)

        persistable_reference = d.pop("persistableReference", UNSET)

        late_bound_crs_request = cls(
            essence=essence,
            persistable_reference=persistable_reference,
        )

        late_bound_crs_request.additional_properties = d
        return late_bound_crs_request
